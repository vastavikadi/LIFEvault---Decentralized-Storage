...
const SignUpPage = () => {
  ...
  const [isHiveKeychain, setIsHiveKeychain] = useState(false); // Toggle between signup methods
  ...
  const handleEmailSignUp = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("https://backend-lifevault.vercel.app/api/auth/signup/email", {
        email,
        password,
      });
    ...
    window.hive_keychain.requestSignBuffer(
      ...
      async (response) => {
        if (response.error) {
          ...
          try {
            const res = await axios.post("https://backend-lifevault.vercel.app/auth/signup/hive", {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-400 to-green-500">
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 bg-white shadow-lg rounded-lg overflow-hidden transform transition duration-500 ease-in-out hover:scale-105">
        ...
        <div className="p-10 flex flex-col justify-center">
          ...
          {/* Toggle between Email/Password and Hive Keychain sign-up */}
